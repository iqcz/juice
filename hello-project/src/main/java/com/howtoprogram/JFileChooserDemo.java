...
public class JFileChooserDemo extends JFrame {
     ...
     * 
     */
    private static final long serialVersionUID = 6997377018686273985L;
    ...
    public void analyzePath() throws IOException {
        ...
        if (path != null && Files.exists(path)) // if exists, display info
        {
            ...
            builder.append(
                    String.format("%s a directory%n", Files.isDirectory(path) ? "Is" : "Is not"));
            builder.append(
                    String.format("%s an absolute path%n", path.isAbsolute() ? "Is" : "Is not"));
