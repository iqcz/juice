    ...
    public void findAcVideoSource(String oldAliKey, String newAliKey) {
        List<String> tableNames = jdbcTemplate
                .queryForList("select table_name from information_schema.tables "
                        + " where table_schema = 'system32' " + " and table_type = 'BASE TABLE' "
                        + " and table_name like 'ac_video_source%' "
                        + " and length(table_name) <= 20 " + " and table_rows > 0", String.class);

        tableNames.stream()
                .filter(tableName -> jdbcTemplate
                        .queryForList("select video_id from ? where url = ?", tableName, oldAliKey)
                        .size() > 0)
                .forEach(tableName -> jdbcTemplate.update("update ? set url = ? where url = ?",
                        tableName, newAliKey, oldAliKey));
    }
*/
}
